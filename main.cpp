...
int main( )
{
	...
	if ( unwind::calculate_stack_size( kernel32_base, "BaseThreadInitThunk", unwind::LOG_VERBOSE ) != BaseThreadInitThunkStackSize )
		return terminate( "incorrect BaseThreadInitThunk stack size" );

	if ( unwind::calculate_stack_size( ntdll_base, "RtlUserThreadStart", unwind::LOG_VERBOSE ) != RtlUserThreadStartStackSize )
	...
	if ( unwind::calculate_stack_size( process_base, add_address, unwind::LOG_VERBOSE ) != AddStackSize )
		return terminate( "incorrect add stack size" );

	if ( unwind::calculate_stack_size( process_base, add2_address, unwind::LOG_VERBOSE ) != Add2StackSize )
